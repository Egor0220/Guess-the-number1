...
<h1>Ты уже предпологал данное значение</h1>
<h1>У тебя осталось <?=$matt?> попыт<?=$end_matt?></h1><br>
<form action="\checks\check_your_int.php" method="post">
    ...
    <br><input placeholder="Введи в поле другое число от <?=$_SESSION['min_ran']?> до <?=$_SESSION['max_ran']?>, которое по вашему мнению загадал компьютер " type="number" min="<?=$min_ran?>" max="<?=$max_ran?>" name="your guess" class="form-control" >
